...
function getCorte(item) {
  return item.corte || "Corte 1";
}

function agruparPorCorte(items) {
  return items.reduce((grupos, item) => {
    const corte = getCorte(item);
    if (!grupos[corte]) grupos[corte] = [];
    grupos[corte].push(item);
    return grupos;
  }, {});
}

function TarjetaEvidencia({ item, viewMode, setModal }) {
  ...
  return (
    ...
    >
      ...
      <div className="ev-body">
        <div className="ev-card-topline">
          <span className="ev-corte-pill">{getCorte(item)}</span>
...
export default function Galeria({
  ...
  const grupos = agruparPorCorte(filtered);
  const ordenCortes = ["Corte 1", "Corte 2"].filter((corte) => grupos[corte]?.length);
  ...
  return (
    <section className="section" id="galeria">
      ...
      <p className="sec-body gallery-intro">
        Las evidencias están organizadas por cortes. El Corte 1 conserva lo que ya estaba hecho y el Corte 2 reúne las nuevas actividades anexadas.
      </p>
      ...
      <div className="search-row">
        <div className="search-box">
          ...
          <input
            type="text"
            placeholder="Buscar por título, categoría, archivo, semana, corte o tags..."
            value={search}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="gallery-cortes">
          {ordenCortes.map((corte) => (
            <div className="gallery-corte-block" key={corte}>
              <div className="gallery-corte-heading">
                <div>
                  <span>{corte}</span>
                  <h3>{corte === "Corte 1" ? "Evidencias construidas inicialmente" : "Actividades anexadas del segundo corte"}</h3>
                </div>
                <strong>{grupos[corte].length}</strong>
              </div>

              <div className={`cards-grid view-${viewMode}`}>
                {grupos[corte].map((item) => (
                  <TarjetaEvidencia
                    key={item.id}
                    item={item}
                    viewMode={viewMode}
